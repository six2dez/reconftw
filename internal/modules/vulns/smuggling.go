...
func (t *SmugglingTask) Run(ctx context.Context, app *appctx.AppContext) (task.Result, error) {
	...
	hostURLs, err := readURLsForSmuggling(app)
	if err != nil && app.Log != nil {
		app.Log.Debug("vulns.smuggling: read urls.jsonl error (best_effort)", "err", err)
	}
	if len(hostURLs) == 0 {
		if app.Log != nil {
			app.Log.Info("vulns.smuggling: no host URLs in artefacts/urls.jsonl — skipping")
		}
...
// readURLsForSmuggling reads artefacts/urls.jsonl and returns all URL strings.
// Returns nil when the file is absent or empty (non-error for SmugglingTask).
func readURLsForSmuggling(app *appctx.AppContext) ([]string, error) {
