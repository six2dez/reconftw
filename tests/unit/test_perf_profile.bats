...
@test "apply_performance_profile sets numeric values for low profile" {
  ...
  # NOT `run`: it executes in a subshell, so the variables the function sets do
  # not survive into this shell and the assertions below would read the unset
  # values this test just unset.
