...
function nuclei_check() {
	...
	if { [[ ! -f "$called_fn_dir/.${FUNCNAME[0]}" ]] || [[ $DIFF == true ]]; } && [[ $NUCLEICHECK == true ]]; then
		...
		file_extensions=(
			# Web files
			"html" "htm" "xhtml" "php" "php3" "php4" "php5" "phtml" "asp" "aspx"
			"jsp" "jspx" "cgi" "cfm" "css" "scss" "less" "js" "ts" "jsx" "tsx"
			# Image files
			"jpg" "jpeg" "png" "gif" "bmp" "tif" "tiff" "svg" "webp" "ico" "psd"
			"ai" "eps" "img"
			# Audio files
			"mp3" "wav" "ogg" "m4a" "flac" "aac" "wma"
			# Video files
			"mp4" "mkv" "flv" "avi" "mov" "wmv" "webm" "ogv" "m4v"
			# Document files
			"txt" "rtf" "doc" "docx" "odt" "pdf" "xls" "xlsx" "ods" "csv" "ppt"
			"pptx" "odp"
			# Archive files
			...
			# Other common files
			"xml" "json" "yaml" "yml" "log" "ini" "conf" "config" "exe" "dll"
			"bin" "iso" "swf" "htc"
		)

		# Function to check if a string ends with a given file extension
		is_file() {
			local path="$1"
			for ext in "${file_extensions[@]}"; do
				if [[ "$path" == *.$ext ]]; then
					return 0  # True: path is a file
				fi
			done
			return 1  # False: path is not a file
		}

		process_url() {
			local url="$1"
			local parsed_url
			parsed_url=$(echo "$url" | awk -F/ '{print $1 "//" $3}')  # Extract scheme and domain
			local path
			path=$(echo "$url" | awk -F/ '{for (i=4; i<=NF; i++) printf "/"$i} END {print ""}')
			local query
			...
			if ! is_file "$path" && [ -z "$query" ]; then
				IFS='/' read -ra parts <<< "$path"
				for i in "${!parts[@]}"; do
					partial_path="${parts[*]:0:$((i+1))}"
					urls+=("$parsed_url/$partial_path/")
				done
			else
				IFS='/' read -ra parts <<< "$path"
				for i in $(seq 1 $(( ${#parts[@]} - 1 ))); do
					partial_path="${parts[*]:0:$i}"
					urls+=("$parsed_url/$partial_path/")
				done
			fi
		...
		while IFS= read -r url; do
			process_url "$url"
		done < "webs/webs_nuclei.txt" > "webs/webs_nuclei_paths.txt"
